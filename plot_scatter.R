...
plot_scatter <- function (mr_results, dat)
{
  ...
  mrres <- plyr::dlply(dat, c("id.exposure", "id.outcome"),
                       function(d) {
                         ...
                         d$beta.exposure[index] <-
                           d$beta.exposure[index] *
                           -1
                         d$beta.outcome[index] <-
                           d$beta.outcome[index] *
                           -1
                         mrres <-
                           subset(mr_results,
                         ...
                         if ("MR Egger (bootstrap)" %in% mrres$method) {
                           ...
                           mrres$a[mrres$method == "MR Egger (bootstrap)"] <-
                             temp$b_i
                         }
                         ggplot2::ggplot(data = d, ggplot2::aes(x = beta.exposure,
                                                                y = beta.outcome)) + ggplot2::geom_errorbar(
                                                                  ggplot2::aes(ymin = beta.outcome -
