...
string_db <- read.csv(paste(data_dir, 'stringdb.txt', sep='/'), sep='\t')
...
exposure_df %>% dplyr::select(protein_external_id) %>% distinct() %>% 
  write.csv(paste(data_dir, 'ensp.csv', sep='/'), row.names = FALSE)

# run mygene_coordinates.py
protein_gene_coordinates <- read.csv(paste(data_dir, 'protein_gene_coordinates.csv', sep='/')) %>% 
...
write.csv(exposure_df, paste(data_dir, 'selected_exposures.csv', sep='/'), row.names = FALSE)
